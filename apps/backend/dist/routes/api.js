...
const checkOutSchema = zod_1.z.object({
    signed_by_host_confirmed: zod_1.z.literal(true, {
        errorMap: () => ({ message: "Bitte bestaetigen Sie die Ansprechpartner-Unterschrift." })
    }),
    returned_badge_number: zod_1.z.string().trim().min(1, "Bitte geben Sie die Besuchsnummer vom Besucherschein ein.").transform((value) => value.toUpperCase())
});
const signatureUpdateSchema = zod_1.z.object({
    ...
    if (value.host_signature_status === visitWorkflow_1.HOST_SIGNATURE_STATUS.SIGNED_LATER && !value.host_signature_date?.trim()) {
        context.addIssue({
    ...
    if (value.host_signature_status === visitWorkflow_1.HOST_SIGNATURE_STATUS.MISSING_EXCEPTION && !value.host_signature_note?.trim()) {
        context.addIssue({
            code: zod_1.z.ZodIssueCode.custom,
...
const guardVisitUpdateSchema = zod_1.z.object({
    ...
    hostDepartment: zod_1.z.string().trim().max(255).optional().or(zod_1.z.literal("")),
    ...
    visitorAddress: zod_1.z.string().trim().max(500).optional().or(zod_1.z.literal("")),
    ...
    idDocumentNumber: zod_1.z.string().trim().max(120).optional().or(zod_1.z.literal("")),
    idDocumentIssuingPlace: zod_1.z.string().trim().max(255).optional().or(zod_1.z.literal("")),
    visitPurposeType: zod_1.z.enum(["private", "business"]).optional().or(zod_1.z.literal("")),
    visitCompanyOrder: zod_1.z.string().trim().max(500).optional().or(zod_1.z.literal("")),
    hostUnit: zod_1.z.string().trim().max(255).optional().or(zod_1.z.literal("")),
    hostBuilding: zod_1.z.string().trim().max(120).optional().or(zod_1.z.literal("")),
    hostRoom: zod_1.z.string().trim().max(80).optional().or(zod_1.z.literal("")),
    hostExtension: zod_1.z.string().trim().max(80).optional().or(zod_1.z.literal("")),
    visitEndType: zod_1.z.enum(["ended", "forwarded"]).optional().or(zod_1.z.literal("")),
    forwardedToNote: zod_1.z.string().trim().max(500).optional().or(zod_1.z.literal("")),
    devicePhotoApp: zod_1.z.boolean().optional(),
    deviceFilmApp: zod_1.z.boolean().optional(),
    deviceVideoCamera: zod_1.z.boolean().optional(),
    deviceManufacturer: zod_1.z.string().trim().max(255).optional().or(zod_1.z.literal("")),
    deviceSerialNumber: zod_1.z.string().trim().max(120).optional().or(zod_1.z.literal("")),
    deviceAccessories: zod_1.z.string().trim().max(500).optional().or(zod_1.z.literal("")),
    deviceDepositNote: zod_1.z.string().trim().max(500).optional().or(zod_1.z.literal("")),
    deviceReturnConfirmed: zod_1.z.boolean().optional(),
    deviceReturnedAt: zod_1.z.string().trim().optional().or(zod_1.z.literal(""))
}).superRefine((value, context) => {
...
const guardCalendarQuerySchema = zod_1.z.object({
    ...
    if (Number.isNaN(from.getTime()) || Number.isNaN(to.getTime())) {
        context.addIssue({ code: zod_1.z.ZodIssueCode.custom, path: ["from"], message: "Ungueltiger Datumsbereich." });
        return;
    }
    if (to < from) {
        context.addIssue({ code: zod_1.z.ZodIssueCode.custom, path: ["to"], message: "Bis-Datum muss nach Von-Datum liegen." });
    }
    const diffDays = Math.floor((to.getTime() - from.getTime()) / (1000 * 60 * 60 * 24));
    if (diffDays > 90) {
        context.addIssue({ code: zod_1.z.ZodIssueCode.custom, path: ["to"], message: "Datumsbereich darf maximal 90 Tage umfassen." });
    }
});
const fieldDefinitionsContextSchema = zod_1.z.enum(["public", "guard", "sibe", "badge", "admin"]);
const adminFieldDefinitionUpdateSchema = zod_1.z.object({
    label: zod_1.z.string().trim().min(1).max(200),
    section: zod_1.z.string().trim().min(1).max(50),
    isActive: zod_1.z.boolean(),
    showInPublic: zod_1.z.boolean(),
    showInGuard: zod_1.z.boolean(),
    showInSibe: zod_1.z.boolean(),
    showOnBadge: zod_1.z.boolean(),
    requiredPublic: zod_1.z.boolean(),
    requiredGuardCheckin: zod_1.z.boolean(),
    requiredBeforePrint: zod_1.z.boolean(),
    sortOrder: zod_1.z.number().int().min(0).max(9999),
    helpText: zod_1.z.string().trim().max(500).optional().nullable().or(zod_1.z.literal("")),
    optionsJson: zod_1.z.string().trim().optional().nullable().or(zod_1.z.literal(""))
}).superRefine((value, context) => {
...
const fieldDefinitionKeySchema = zod_1.z.string().trim().regex(/^[a-z][a-z0-9_]{1,99}$/, "Ungueltiger fieldKey.");
const allowedFieldTypes = ["text", "textarea", "date", "email", "phone", "select", "checkbox", "number"];
const allowedSections = ["Besucher", "Adresse", "Ansprechpartner", "Besuch", "Ausweis", "Ziel/Raum", "Geraete", "Sonstiges"];
const adminFieldDefinitionCreateSchema = zod_1.z.object({
    label: zod_1.z.string().trim().min(1).max(200),
    fieldType: zod_1.z.enum(allowedFieldTypes),
    section: zod_1.z.enum(allowedSections),
    isActive: zod_1.z.boolean().optional().default(true),
    showInPublic: zod_1.z.boolean().optional().default(false),
    showInGuard: zod_1.z.boolean().optional().default(true),
    showInSibe: zod_1.z.boolean().optional().default(true),
    showOnBadge: zod_1.z.boolean().optional().default(false),
    requiredPublic: zod_1.z.boolean().optional().default(false),
    requiredGuardCheckin: zod_1.z.boolean().optional().default(false),
    requiredBeforePrint: zod_1.z.boolean().optional().default(false),
    sortOrder: zod_1.z.number().int().min(0).max(9999).optional().default(100),
    helpText: zod_1.z.string().trim().max(500).optional().nullable().or(zod_1.z.literal("")),
    optionsJson: zod_1.z.string().trim().max(8000).optional().nullable().or(zod_1.z.literal("")),
    fieldKey: fieldDefinitionKeySchema.optional()
});
const fieldConfigImportFieldSchema = zod_1.z.object({
    ...
    requiredBeforePrint: zod_1.z.boolean(),
    sortOrder: zod_1.z.number().int().min(0).max(9999),
    helpText: zod_1.z.string().trim().max(500).nullable().optional(),
    options: zod_1.z.unknown().nullable().optional()
}).superRefine((value, context) => {
    if (!value.showInPublic && value.requiredPublic) {
        context.addIssue({ code: zod_1.z.ZodIssueCode.custom, path: ["requiredPublic"], message: "requiredPublic erfordert showInPublic=true." });
    }
    if (!value.showInGuard && (value.requiredGuardCheckin || value.requiredBeforePrint)) {
        context.addIssue({ code: zod_1.z.ZodIssueCode.custom, path: ["requiredGuardCheckin"], message: "Guard-Pflichten erfordern showInGuard=true." });
    }
});
const fieldConfigImportSchema = zod_1.z.object({
    schema: zod_1.z.literal("besucher-manager-field-config"),
    version: zod_1.z.literal(1),
    exportedAt: zod_1.z.string().optional(),
    app: zod_1.z.string().optional(),
    fields: zod_1.z.array(fieldConfigImportFieldSchema).min(1)
});
...
function buildFieldKeyFromLabel(label) {
    return label
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "_")
        .replace(/^_+|_+$/g, "")
...
exports.apiRouter.get("/api/guard/visits/calendar", async (request, response) => {
    const user = await requireRole(request, response, ["admin", "guard"]);
    if (!user)
        return;
    ...
    try {
        ...
        const visits = await (0, guardVisits_1.getCalendarVisitsForUser)(user, {
            from,
    ...
    try {
        ...
        if (error instanceof Error) {
            ...
            if (error.message === "visit_required_fields_missing") {
                return sendError(response, 400, "VALIDATION_ERROR", "Vor dem Check-in muessen fehlende Pflichtdaten ergaenzt werden.", error.details);
            }
        }
...
exports.apiRouter.post("/api/guard/visits/:id/check-out", async (request, response) => {
    ...
    try {
        await (0, guardVisits_1.checkOutVisit)(user, request.params.id, result.data.returned_badge_number, {
            status: visitWorkflow_1.HOST_SIGNATURE_STATUS.SIGNED_SAME_DAY
        }, undefined, getRequestIp(request), getRequestUserAgent(request));
        return response.json({
            success: true,
...
exports.apiRouter.put("/api/guard/visits/:id/signature", async (request, response) => {
    const user = await requireRole(request, response, ["admin", "guard"]);
    if (!user) {
        return;
    }
    ...
    try {
        await (0, guardVisits_1.updateHostSignatureForGuard)(user, request.params.id, {
...
exports.apiRouter.get("/api/sibe/summary", async (request, response) => {
    ...
    try {
        ...
        const [visitorsTotal, activeVisitors, todaysVisits, checkedInVisitors, usersTotal, activeUsers, signaturesPending, signaturesFollowUp, signaturesExceptions] = await Promise.all([
            ...
            pool.request().query(`SELECT COUNT(*) AS count FROM dbo.visits WHERE ISNULL(host_signature_status, '${visitWorkflow_1.HOST_SIGNATURE_STATUS.PENDING}') = '${visitWorkflow_1.HOST_SIGNATURE_STATUS.PENDING}'`),
            pool.request().query(`SELECT COUNT(*) AS count FROM dbo.visits WHERE host_signature_status = '${visitWorkflow_1.HOST_SIGNATURE_STATUS.SIGNED_LATER}'`),
            pool.request().query(`SELECT COUNT(*) AS count FROM dbo.visits WHERE host_signature_status = '${visitWorkflow_1.HOST_SIGNATURE_STATUS.MISSING_EXCEPTION}'`)
        ]);
        return response.json({
...
exports.apiRouter.get("/api/sibe/statistics/visits", async (request, response) => {
    const user = await requireRole(request, response, ["admin", "sibe"]);
    if (!user)
        return;
    ...
    try {
        ...
        const requestBuilder = pool.request()
            .input("fromDate", mssql_1.default.Date, from)
            .input("toDate", mssql_1.default.Date, to);
        const summaryResult = await requestBuilder.query(`
      SELECT
        COUNT(*) AS total,
        SUM(CASE WHEN status = '${visitWorkflow_1.VISIT_STATUS.PRE_REGISTERED}' OR status = 'vorangemeldet' THEN 1 ELSE 0 END) AS preRegistered,
        SUM(CASE WHEN status = '${visitWorkflow_1.VISIT_STATUS.CHECKED_IN}' OR status = 'eingecheckt' THEN 1 ELSE 0 END) AS checkedIn,
        SUM(CASE WHEN status = '${visitWorkflow_1.VISIT_STATUS.CHECKED_OUT}' OR status = 'ausgecheckt' THEN 1 ELSE 0 END) AS checkedOut
      FROM dbo.visits
...
exports.apiRouter.get("/api/sibe/visitors", async (request, response) => {
    ...
    try {
        ...
        if (company) {
            requestBuilder.input("company", mssql_1.default.NVarChar(255), `%${company}%`);
            whereClause += " AND v.company LIKE @company";
        }
        if (from) {
            requestBuilder.input("fromDate", mssql_1.default.DateTime2, new Date(from));
            whereClause += " AND EXISTS (SELECT 1 FROM dbo.visits vi_from WHERE vi_from.visitor_id = v.id AND vi_from.valid_from >= @fromDate)";
        }
        if (to) {
            requestBuilder.input("toDate", mssql_1.default.DateTime2, new Date(to));
...
exports.apiRouter.get("/api/sibe/visits", async (request, response) => {
    ...
    try {
      ...
      CASE
        WHEN vt.status = 'vorangemeldet' THEN '${visitWorkflow_1.VISIT_STATUS.PRE_REGISTERED}'
        WHEN vt.status = 'eingecheckt' THEN '${visitWorkflow_1.VISIT_STATUS.CHECKED_IN}'
        WHEN vt.status = 'ausgecheckt' THEN '${visitWorkflow_1.VISIT_STATUS.CHECKED_OUT}'
        ELSE vt.status
      END
    `;
        if (search) {
        ...
        if (status && status !== "all") {
            if (status === "overdue") {
                conditions.push(`
          ${normalizedVisitStatusSql} IN ('${visitWorkflow_1.VISIT_STATUS.PRE_REGISTERED}', '${visitWorkflow_1.VISIT_STATUS.CHECKED_IN}')
        ...
        if (gate) {
            requestBuilder.input("gate", mssql_1.default.NVarChar(255), `%${gate}%`);
        ...
        if (hostName) {
            requestBuilder.input("hostName", mssql_1.default.NVarChar(255), `%${hostName}%`);
            conditions.push("vt.host_name LIKE @hostName");
        }
        if (licensePlate) {
            requestBuilder.input("licensePlate", mssql_1.default.NVarChar(80), `%${licensePlate}%`);
            conditions.push("ISNULL(vt.license_plate, '') LIKE @licensePlate");
        }
        if (badgeNumber) {
            requestBuilder.input("badgeNumber", mssql_1.default.NVarChar(80), `%${badgeNumber}%`);
...
exports.apiRouter.get("/api/sibe/users", async (request, response) => {
    ...
    try {
        ...
        if (username) {
            requestBuilder.input("username", mssql_1.default.NVarChar(255), `%${username}%`);
        ...
        if (lastLoginFrom) {
            requestBuilder.input("lastLoginFrom", mssql_1.default.DateTime2, new Date(lastLoginFrom));
            conditions.push("u.last_login_at >= @lastLoginFrom");
        }
        if (lastLoginTo) {
            requestBuilder.input("lastLoginTo", mssql_1.default.DateTime2, new Date(lastLoginTo));
...
exports.apiRouter.get("/api/field-definitions", async (request, response) => {
    ...
    try {
        const definitions = parsed.data === "admin"
            ? await (0, fieldDefinitions_1.listAdminFieldDefinitions)()
            : await (0, fieldDefinitions_1.listFieldDefinitions)(parsed.data);
...
exports.apiRouter.get("/api/admin/field-definitions", async (request, response) => {
    const user = await requireRole(request, response, ["admin"]);
    if (!user) {
        return;
    }
    try {
        const definitions = await (0, fieldDefinitions_1.listAdminFieldDefinitions)();
        return response.json({ definitions });
    }
    catch (error) {
        return handleUnexpectedError(response, error, "DATABASE_ERROR", "Felddefinitionen konnten nicht geladen werden.");
...
exports.apiRouter.get("/api/admin/field-definitions/export", async (request, response) => {
    const user = await requireRole(request, response, ["admin"]);
    if (!user)
        return;
    try {
...
exports.apiRouter.post("/api/admin/field-definitions", async (request, response) => {
    const user = await requireRole(request, response, ["admin"]);
    if (!user)
        return;
    ...
    try {
        ...
        const exists = await pool.request()
            .input("fieldKey", mssql_1.default.NVarChar(100), fieldKey)
            .query("SELECT COUNT(1) AS count FROM dbo.field_definitions WHERE field_key = @fieldKey");
        ...
        await pool.request()
            .input("id", mssql_1.default.UniqueIdentifier, node_crypto_1.default.randomUUID())
            .input("fieldKey", mssql_1.default.NVarChar(100), fieldKey)
            .input("label", mssql_1.default.NVarChar(200), payload.label)
            .input("fieldType", mssql_1.default.NVarChar(50), payload.fieldType)
            ...
            .input("requiredPublic", mssql_1.default.Bit, payload.showInPublic ? payload.requiredPublic : false)
            .input("requiredGuardCheckin", mssql_1.default.Bit, payload.showInGuard ? payload.requiredGuardCheckin : false)
            .input("requiredBeforePrint", mssql_1.default.Bit, payload.showInGuard ? payload.requiredBeforePrint : false)
            .input("sortOrder", mssql_1.default.Int, payload.sortOrder)
            .input("helpText", mssql_1.default.NVarChar(500), payload.helpText || null)
            .input("optionsJson", mssql_1.default.NVarChar(mssql_1.default.MAX), payload.optionsJson || null)
            .query(`
...
exports.apiRouter.post("/api/admin/field-definitions/import/preview", async (request, response) => {
    const user = await requireRole(request, response, ["admin"]);
    if (!user)
        return;
    ...
    try {
        const pool = await (0, db_1.getPool)();
        const existing = await pool.request().query("SELECT field_key AS fieldKey FROM dbo.field_definitions");
...
exports.apiRouter.post("/api/admin/field-definitions/import", async (request, response) => {
    const user = await requireRole(request, response, ["admin"]);
    if (!user)
        return;
    const parsed = fieldConfigImportSchema.safeParse(request.body);
        ...
        for (const field of payload.fields) {
            ...
            if (existing) {
                ...
                await pool.request()
                    ...
                    .input("label", mssql_1.default.NVarChar(200), field.label)
                    .input("section", mssql_1.default.NVarChar(50), field.section)
                    .input("isActive", mssql_1.default.Bit, field.isActive)
                    .input("showInPublic", mssql_1.default.Bit, field.showInPublic)
                    .input("showInGuard", mssql_1.default.Bit, field.showInGuard)
                    .input("showInSibe", mssql_1.default.Bit, field.showInSibe)
                    .input("showOnBadge", mssql_1.default.Bit, field.showOnBadge)
                    .input("requiredPublic", mssql_1.default.Bit, field.showInPublic ? field.requiredPublic : false)
                    .input("requiredGuardCheckin", mssql_1.default.Bit, field.showInGuard ? field.requiredGuardCheckin : false)
                    .input("requiredBeforePrint", mssql_1.default.Bit, field.showInGuard ? field.requiredBeforePrint : false)
                    .input("sortOrder", mssql_1.default.Int, field.sortOrder)
                    .input("helpText", mssql_1.default.NVarChar(500), field.helpText || null)
                    .input("optionsJson", mssql_1.default.NVarChar(mssql_1.default.MAX), optionsJson)
                    .input("fieldType", mssql_1.default.NVarChar(50), fieldTypeForUpdate)
                    .query(`
          ...
          `);
                ...
                await pool.request()
                    .input("id", mssql_1.default.UniqueIdentifier, node_crypto_1.default.randomUUID())
                    .input("fieldKey", mssql_1.default.NVarChar(100), field.fieldKey)
                    .input("label", mssql_1.default.NVarChar(200), field.label)
                    .input("fieldType", mssql_1.default.NVarChar(50), field.fieldType)
                    .input("section", mssql_1.default.NVarChar(50), field.section)
                    .input("isActive", mssql_1.default.Bit, field.isActive)
                    .input("showInPublic", mssql_1.default.Bit, field.showInPublic)
                    .input("showInGuard", mssql_1.default.Bit, field.showInGuard)
...
exports.apiRouter.put("/api/admin/field-definitions/:id", async (request, response) => {
    const user = await requireRole(request, response, ["admin"]);
    if (!user) {
        return;
    }
    ...
    try {
        await (0, fieldDefinitions_1.updateFieldDefinition)(request.params.id, {
            ...parsed.data,
