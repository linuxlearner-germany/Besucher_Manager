...
test("simplified visit policy endpoint allows SiBe", async () => {
  const access = await checkAccess("sibe");
  assert.equal(access.user?.role, "sibe");
  assert.equal(access.statusCode, null);
});

test("simplified visit policy endpoint rejects admin and every other authenticated role with 403", async () => {
  for (const role of ["admin", "guard", "kaskdt", "custom"] as const) {
    ...
    assert.deepEqual(access.body, { error: "FORBIDDEN", message: "Nicht ausreichend berechtigt." }, role);
  }
});

test("simplified visit policy endpoint rejects unauthenticated requests with 401", async () => {
  const access = await checkAccess(null);
  assert.equal(access.user, null);
  assert.equal(access.statusCode, 401);
  assert.deepEqual(access.body, { error: "UNAUTHORIZED", message: "Anmeldung erforderlich." });
});

test("the simplified visitor write endpoint uses the SiBe-only role guard", () => {
  const source = readFileSync(resolve(__dirname, "sibe.ts"), "utf8");
  const endpoint = source.match(/sibeRouter\.post\("\/api\/sibe\/visitors"[\s\S]*?const parsed = simplifiedSibeVisitorSchema/);
  assert.ok(endpoint, "simplified visitor endpoint must exist");
  assert.match(endpoint[0], /requireRole\(request, response, \["sibe"\]\)/);
  assert.doesNotMatch(endpoint[0], /requirePermission|requireAnyPermission/);
});
