...
async function sendVerificationMail(to: string, reference: string, token: string, expiresAt: Date): Promise<boolean> {
  ...
  return sendWorkflowMail({ to:[to], subject:"Bitte bestätigen Sie Ihre E-Mail-Adresse", text:[`Besucherantrag ${reference}`,"Bitte bestätigen Sie Ihre E-Mail-Adresse, damit der Antrag an KSKdt weitergeleitet werden kann.",url,`Gültig bis: ${expiresAt.toLocaleString("de-DE",{timeZone:"Europe/Berlin"})}`].join("\n\n"), html:buildMailHtml({heading:"E-Mail-Adresse bestätigen",introduction:"Bitte bestätigen Sie Ihre E-Mail-Adresse, damit der Besucherantrag an KSKdt weitergeleitet werden kann.",details:[{label:"Referenz",value:reference},{label:"Link gültig bis",value:expiresAt.toLocaleString("de-DE",{timeZone:"Europe/Berlin"})}],detailUrl:url,detailLinkLabel:"E-Mail-Adresse bestätigen"}) });
}
...
function buildOutboxMail(type:string,p:any):{subject:string;text:string;html:string}{
  if(type==="kaskdt_new"){const url=`${env.PUBLIC_BASE_URL.replace(/\/+$/,'')}/kaskdt/antraege/${p.applicationId}`;return{subject:"Neuer Besucherantrag eingegangen",text:`Referenz: ${p.reference}\nAntragsteller: ${p.applicantName||"–"}\nE-Mail: ${p.applicantEmail}\nOrganisation: ${p.organization||"–"}\nPersonen: ${p.count}\nZeitraum: ${p.period}\n${url}`,html:buildMailHtml({heading:"Neuer Besucherantrag eingegangen",introduction:"Ein öffentlicher XLSX-Antrag steht zur Prüfung bereit.",details:[{label:"Referenz",value:p.reference},{label:"Antragsteller",value:p.applicantName},{label:"E-Mail",value:p.applicantEmail},{label:"Organisation",value:p.organization},{label:"Personen",value:String(p.count)},{label:"Zeitraum",value:p.period}],detailUrl:url,detailLinkLabel:"Antrag prüfen"})};}
  if(type==="decision"){const approved=p.entries.filter((e:any)=>e.status==="approved"),rejected=p.entries.filter((e:any)=>e.status==="rejected");const heading=approved.length&&rejected.length?"Ihr Besucherantrag wurde teilweise genehmigt.":approved.length?"Ihr Besucherantrag wurde genehmigt.":"Ihr Besucherantrag wurde abgelehnt.";const lines=[heading,`Referenz: ${p.reference}`,"",...(approved.length?["Genehmigt",...approved.map((e:any)=>`- ${[e.firstName,e.lastName].filter(Boolean).join(" ")||"Keine Angabe"}`),""]:[]),...(rejected.length?["Abgelehnt",...rejected.map((e:any)=>`- ${[e.firstName,e.lastName].filter(Boolean).join(" ")||"Keine Angabe"} – ${e.rejectionReason||"ohne Angabe"}`)]:[])];return{subject:heading,text:lines.join("\n"),html:`<h2>${escapeMailHtml(heading)}</h2><p>Referenz: ${escapeMailHtml(p.reference)}</p>${approved.length?`<h3>Genehmigt</h3><ul>${approved.map((e:any)=>`<li>${escapeMailHtml([e.firstName,e.lastName].filter(Boolean).join(" ")||"Keine Angabe")}</li>`).join("")}</ul>`:""}${rejected.length?`<h3>Abgelehnt</h3><ul>${rejected.map((e:any)=>`<li>${escapeMailHtml([e.firstName,e.lastName].filter(Boolean).join(" ")||"Keine Angabe")} – ${escapeMailHtml(e.rejectionReason||"ohne Angabe")}</li>`).join("")}</ul>`:""}`};}
  return{subject:"Ihr Besucherantrag wurde eingereicht",text:`Ihr Besucherantrag wurde eingereicht.\nReferenz: ${p.reference}\nPersonen: ${p.count}\nZeitraum: ${p.period}\nDer Antrag wird von KSKdt geprüft.`,html:buildMailHtml({heading:"Besucherantrag eingereicht",introduction:"Ihr Antrag wurde eingereicht und wird von KSKdt geprüft.",details:[{label:"Referenz",value:p.reference},{label:"Personen",value:String(p.count)},{label:"Zeitraum",value:p.period}]})};
}
