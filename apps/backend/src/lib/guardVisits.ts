...
export function getVisitCompleteness(visit: {
  status: string;
  firstName: string;
  lastName: string;
  company: string;
  ...
  if (isBlank(visit.firstName)) missingRequiredFields.push("Vorname");
  if (isBlank(visit.lastName)) missingRequiredFields.push("Nachname");
  if (isBlank(visit.company)) missingRequiredFields.push("Firma / Organisation");
  if (isBlank(visit.hostName)) missingRequiredFields.push("Ansprechpartner");
  if (isBlank(visit.hostPhone)) missingRequiredFields.push("Ansprechpartner Telefon");
  if (isBlank(visit.purpose)) missingRequiredFields.push("Besuchszweck");
  if (isBlank(visit.validFrom)) missingRequiredFields.push("Gueltig von");
  if (isBlank(visit.validUntil)) missingRequiredFields.push("Gueltig bis");
  ...
  if (!hasAddressFreeText && !hasStructuredAddressComplete) {
    ...
    if (hasAnyStructuredAddressPart) {
      if (!hasStreet) missingRequiredFields.push("Strasse");
      if (!hasHouseNumber) missingRequiredFields.push("Hausnummer");
      if (!hasPostalCode) missingRequiredFields.push("PLZ");
      if (!hasCity) missingRequiredFields.push("Wohnort");
    }
  }

  if (isBlank(visit.idDocumentType)) missingRequiredFields.push("Ausweisart");
  if (isBlank(visit.idDocumentValidUntil)) missingRequiredFields.push("Ausweis gueltig bis");
  if (isBlank(visit.idDocumentNumber)) missingRequiredFields.push("Ausweisnummer");
  if (isBlank(visit.idDocumentIssuingPlace)) missingRequiredFields.push("Ausstellungsort");
  ...
  if (!visit.gateId && visit.status === VISIT_STATUS.PRE_REGISTERED) {
  ...
  if (isBlank(visit.birthDate)) infos.push({ field: "birth_date", message: "Geburtsdatum ist nicht angegeben.", severity: "info" });
  if (isBlank(visit.visitorPhone)) infos.push({ field: "visitor_phone", message: "Besucher-Telefon ist nicht angegeben.", severity: "info" });
  if (isBlank(visit.visitorEmail)) infos.push({ field: "visitor_email", message: "Besucher-E-Mail ist nicht angegeben.", severity: "info" });
  if (isBlank(visit.licensePlate)) infos.push({ field: "license_plate", message: "Kennzeichen ist nicht angegeben.", severity: "info" });
  if (isBlank(visit.idDocumentType) && isBlank(visit.idDocumentNumber)) infos.push({ field: "id_document", message: "Ausweisdaten wurden nicht erfasst.", severity: "info" });

  return {
    canCheckIn: visit.status === VISIT_STATUS.PRE_REGISTERED && errors.length === 0,
    canPrintBadge: visit.status === VISIT_STATUS.PRE_REGISTERED || visit.status === VISIT_STATUS.CHECKED_IN || visit.status === VISIT_STATUS.CHECKED_OUT,
