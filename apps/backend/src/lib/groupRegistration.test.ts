...
function loadGroupRegistrationModule() {
  ...
  return require("./groupRegistration") as typeof import("./groupRegistration");
}

test("group registration preserves German dates as day-month-year", () => {
  const { normalizeGroupDateOnly } = loadGroupRegistrationModule();

  assert.equal(normalizeGroupDateOnly("05.08.2026"), "2026-08-05");
  assert.equal(normalizeGroupDateOnly("31.12.2030"), "2030-12-31");
  assert.equal(normalizeGroupDateOnly("31.02.2026"), null);
});

test("group rows use the public required-field and format validation", () => {
  const { validateGroupPreRegistrationRows } = require("./publicPreRegistrationSchema") as typeof import("./publicPreRegistrationSchema");
  const errors = validateGroupPreRegistrationRows([{
    sourceRowNumber: 2,
    firstName: "Max",
    lastName: "Muster",
    visitorStreet: "",
